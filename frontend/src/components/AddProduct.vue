<template>
  <div class="drt">
    <label class="label">Бронирование№ </label>
    <div class="field">
  ...
  <span v-if="!clientMail || !isValidEmail(clientMail)">Пожалуйста, введите корректный адрес электронной почты.</span>
    ...
    <div class="control">
      <router-link :to="{ name: 'Index' }" class="button is-danger">Назад</router-link>
      <button margin-right:100px class="button is-success" @click="saveClient" v-bind:disabled="!clientPId" >Бронировать</button>
    </div>
  </div>
</template>
